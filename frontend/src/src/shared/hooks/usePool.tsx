...
export const usePool = (): UsePoolReturn => {
  ...
  const fetchPools = useCallback(async () => {
    ...
    try {
      const response = await fetch(
        `${import.meta.env.VITE_API_BASE_URL}/pool/investor/${user.id}`
      );
  ...
  const createPool = useCallback(
    async (data: CreatePoolData): Promise<Pool> => {
      setIsLoading(true);
      ...
      try {
        ...
        const response = await fetch(
          `${import.meta.env.VITE_API_BASE_URL}/pool`,
          {
            method: "POST",
  ...
  const updatePool = useCallback(
    async (poolId: string, updates: Partial<Pool>) => {
      setIsLoading(true);

      try {
        const response = await fetch(
          `${import.meta.env.VITE_API_BASE_URL}/pool/${poolId}`,
          {
            method: "PUT",
            headers: { "Content-Type": "application/json" },
  ...
  const pausePool = useCallback(
    async (poolId: string) => {
      setIsLoading(true);

      try {
        const response = await fetch(
          `${import.meta.env.VITE_API_BASE_URL}/pool/${poolId}/status`,
          {
            method: "PUT",
            headers: { "Content-Type": "application/json" },
  ...
  const increaseCapital = useCallback(
  ...
  const getPoolStats = useCallback(
    (poolId: string) => {
      ...
      const allocatedCapital =
        pool.allocatedAmount || pool.allocatedCapital || 0;
      const currentLoansCount =
        pool.currentLoansCount || poolAllocations.length;

      const totalReturn = poolAllocations.reduce(
        (sum, alloc) => sum + alloc.totalPaid,
        0
      );
      ...
      const avgLoanSize =
        currentLoansCount > 0 ? allocatedCapital / currentLoansCount : 0;
  ...
  const getEligibleBorrowers = useCallback(
    (criteria: Pool["criteria"]): number => {
      ...
      else if (criteria.minScore >= 750) baseEligible *= 0.3;
      else if (criteria.minScore >= 700) baseEligible *= 0.45;
      else if (criteria.minScore >= 650) baseEligible *= 0.6;
      ...
      if (criteria.minInterestRate >= 25) baseEligible *= 0.6;

...
interface MaskedInputProps extends Omit<React.InputHTMLAttributes<HTMLInputElement>, 'type' | 'onChange'> {
  mask: 'cpf' | 'cnpj' | 'phone' | 'currency' | 'money' | 'date' | 'usd' | 'creditCard' | 'expiryDate' | 'cvv';
  value?: string;
...
export const MaskedInput = forwardRef<HTMLInputElement, MaskedInputProps>(
  ({ mask, value = '', onChange, isValid, className = '', placeholder, currency = 'BRL', ...props }, ref) => {
    ...
    const applyMask = (inputValue: string) => {
      ...
      if (mask === 'cpf' && numbers.length > 11) {
        ...
        if (cnpjConfig && cnpjConfig.pattern) {
          ...
          if (match) {
            return cnpjConfig.format.replace(/\$(\d+)/g, (_, index) => match[parseInt(index)] || '');
          }
        }
      }
      ...
      if (maskConfig.pattern) {
        ...
        if (match) {
          return maskConfig.format.replace(/\$(\d+)/g, (_, index) => match[parseInt(index)] || '');
        }
      }
    ...
    const borderColor = isValid === undefined 
      ? 'border-gray-600' 
      : isValid 
        ? 'border-green-500' 
        : 'border-red-500';
